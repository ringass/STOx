...
class Robot(Drawable, Updatable):
    ...
    def update(self) -> None:
        ...
        if self.__goal is None or abs(self.__pos - self.__goal) < self.tolerance:
            ...
            if self.subgoal is None and is_line_intersecting_obstacle(self.__pos, self.__goal, obstacle, obstacle_radius):
    ...
    def draw(self, screen: Surface) -> None:
        ...
        top_front:Vector2D = center + Vector2D.from_polar(self.__radius, math.pi/6+self.__angle)
        bottom_front:Vector2D = center + Vector2D.from_polar(self.__radius, -math.pi/6+self.__angle)
        pygame.draw.polygon(screen,  (255,255,255), [center.to_tuple(), top_front.to_tuple(), bottom_front.to_tuple()])
