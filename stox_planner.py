...
def is_line_intersecting_obstacle(start: Vector2D, goal: Vector2D, obstacle: Vector2D, obstacle_radius: float) -> bool:
...
def calculate_subgoal(start: Vector2D, goal: Vector2D, obstacle: Vector2D, obstacle_radius: float, margin: float = 60.0) -> Vector2D:
    line_goal = goal - start
    norm_line_goal = line_goal * (1 / abs(line_goal))  
    ...
    proj_len = (line_obstacle.X * norm_line_goal.X + line_onstacle.Y * norm_line_goal.Y)
